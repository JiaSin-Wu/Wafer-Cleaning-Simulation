...
int main(int argc, char **argv) {
    ...
    // Wafer Speeds: 10 distinct speeds
    ...
    // New variables to track the minimum RMS
    double min_rms_deviation = numeric_limits<double>::max();
    double best_wafer_rotation = 0.0;
    double best_brush_rotation = 0.0;
    int best_sim_count = 0;
    
    int sim_count = 0;
    int total_sims = wafer_rpms.size() * brush_rpms.size();

    // The entire simulation process can be conceptualized as a Wafer Cleaning System:
    // 1. Load Brush Topology (Input)
    // 2. Wafer Rotation + Brush Sweeping (Cleaning Kernel)
    // 3. RMS Evaluation (Output metric)
    ...
    for (double wafer_rotation : wafer_rpms) {
        for (double brush_rotation : brush_rpms) {
            ...
            // Track the minimum RMS deviation
            if (rms_deviation < min_rms_deviation) {
                min_rms_deviation = rms_deviation;
                best_wafer_rotation = wafer_rotation;
                best_brush_rotation = brush_rotation;
    ...
    // Print the final minimum RMS result
    cout << "\n============================================" << endl;
    cout << "Best Cleaning Result Found" << endl;
    cout << "Minimum RMS Deviation: " << min_rms_deviation << endl;
    cout << "Found in Simulation: " << best_sim_count << endl;
    cout << "Optimal Wafer Rotation: " << best_wafer_rotation << " rpm" << endl;
    cout << "Optimal Brush Rotation: " << best_brush_rotation << " rpm" << endl;
    cout << "============================================" << endl;
